...
void WINAPI OnSendMessage( PTP_CALLBACK_INSTANCE pInstance, void* packet)
{
	...
	if(option == SEND)
	{
		...
		bsIn.Read( toUser);
		bsIn.Read( message);
		//处理发送消息,发送人：fromUser ， 接收人：toUser ， 消息内容：message
		char sqlCmd[1024];
		sprintf( sqlCmd, "INSERT INTO message (fromuser,touser,message,isread,t) values (%d,%d,'%s',0,%d)"
			, fromUser, toUser, message.C_String(),t);
