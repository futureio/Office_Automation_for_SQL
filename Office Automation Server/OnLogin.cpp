...
void WINAPI OnLogin( PTP_CALLBACK_INSTANCE pInstance, void* packet)
{
  ...
  int ID;
  char sqlCmd[1024];
  sprintf( sqlCmd, "SELECT id,department,priority FROM account WHERE name='%s' AND password='%s'"
	  , username.C_String(), password.C_String());
  ...
  if( count != 0)
  {
    ...
    //发送暂存即时消息信息
	  ...
	  //下发未读，已读公文。
	  sprintf( sqlMail, 
		  "SELECT fromuser,todepartment,title,t,content,isread,type from mail where type=1 and todepartment=%d ",ID);
	  g_globalData->databaseLock.Lock();
	  ...
	  //下发发送公文
	  sprintf( sqlMail, 
		  "SELECT fromuser,todepartment,title,t,content,isread,type from mail where type=0 and fromuser=%d ",ID);
	  g_globalData->databaseLock.Lock();
	  mailNum = g_globalData->database->QueryWithResult(sqlMail);
	  if(mailNum != 0)
	  {
