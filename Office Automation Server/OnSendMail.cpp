...
void WINAPI OnSendMail( PTP_CALLBACK_INSTANCE pInstance, void* packet)
{
	...
	if(option == MAILCREATE)//起草公文
	{
		...
		//获取组长id
		sprintf( sqlCmd, 
			"select department_master from department where id in (select department from account where id=%d)",fromUser);
	...
	{
		sprintf( sqlCmd, "UPDATE mail SET isread=2 where fromuser=%d and t=%d and todepartment=%d and type=%d"
			, fromUser,t,toDepartment,type);
		g_globalData->databaseLock.Lock();
		g_globalData->database->QueryWithoutResult(sqlCmd);
		...
		{
			sprintf( sqlAdd, 
				"INSERT INTO mail (fromuser,todepartment,title,t,content,isread,type) values (%d,%d,'%s',%d,'%s',4,1)"
													,fromUser,toID[n],title.C_String(),t,content.C_String());
			g_globalData->database->QueryWithoutResult(sqlAdd);
			//推送消息
			map<int, RakNet::SystemAddress>::iterator iter;
	...
	{
		sprintf( sqlCmd, "UPDATE mail SET isread=3 where fromuser=%d and t=%d and todepartment=%d and type=%d"
			, fromUser,t,toDepartment,type);
		g_globalData->databaseLock.Lock();
		g_globalData->database->QueryWithoutResult(sqlCmd);
		g_globalData->databaseLock.Unlock();
		...
		else{printf("%d公文(3)未发送\n",fromUser);}
	}
	else if(option == MAILSEND)//公文设置未读
	{
		int toUser = toDepartment;
		printf("公文(4)\n");
		sprintf( sqlCmd, "UPDATE mail SET isread=4 where fromuser=%d and t=%d and todepartment=%d and type=%d"
			, fromUser,t,toDepartment,type);
		g_globalData->databaseLock.Lock();
		g_globalData->database->QueryWithoutResult(sqlCmd);
		g_globalData->databaseLock.Unlock();
	...
	else if(option == DONE)//公文已读
	{
		int toUser = toDepartment;
		printf("公文(5)\n");
		sprintf( sqlCmd, "UPDATE mail SET isread=5 where fromuser=%d and t=%d and todepartment=%d and type=%d"
			, fromUser,t,toDepartment,type);
		g_globalData->databaseLock.Lock();
		g_globalData->database->QueryWithoutResult(sqlCmd);
		g_globalData->databaseLock.Unlock();
	...
	{
		printf("公文(6)\n");
		sprintf( sqlCmd, "UPDATE mail SET isread=6 where fromuser=%d and t=%d and todepartment=%d and type=%d"
			, fromUser,t,toDepartment,type);
		g_globalData->databaseLock.Lock();
		g_globalData->database->QueryWithoutResult(sqlCmd);
		g_globalData->databaseLock.Unlock();
	...
	{
		printf("公文(7)\n");
		sprintf( sqlCmd, "UPDATE mail SET isread=7 where fromuser=%d and t=%d and todepartment=%d and type=%d"
			, fromUser,t,toDepartment,type);
		g_globalData->databaseLock.Lock();
		g_globalData->database->QueryWithoutResult(sqlCmd);
		g_globalData->databaseLock.Unlock();
